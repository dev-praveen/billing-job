...
public class BillingJobConfiguration {
  ...
  public JdbcCursorItemReader<BillingData> billingDataTableReader(DataSource dataSource) {
    String sql = "select * from mydb.BILLING_DATA";
  ...
  public JdbcBatchItemWriter<BillingData> billingDataTableWriter(DataSource dataSource) {
    ...
    String sql =
        "insert into mydb.BILLING_DATA values (:dataYear, :dataMonth, :accountId, :phoneNumber, :dataUsage, :callDuration, :smsCount)";
  ...
  public Step step3(
    ...
    return new StepBuilder("reportGeneration", jobRepository)
        .<BillingData, ReportingData>chunk(10, transactionManager)
  ...
  public Job job(JobRepository jobRepository, Step step1, Step step2, Step step3) {
    return new JobBuilder("BillingJob", jobRepository).start(step1).next(step2).next(step3).build();
  }
}
